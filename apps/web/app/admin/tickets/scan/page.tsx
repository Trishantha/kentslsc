...
export default function TicketScanPage() {
  ...
  const checkIn = useMutation({
    ...
    onSuccess: () => {
      // Scanner stays paused; success state is shown.
    }
  });
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="section-title flex items-center gap-2">
        <QrCode className="h-6 w-6 text-neon-blue" />
        Ticket Scan
      </h1>
      <p className="mt-2 text-slate-600 dark:text-slate-400">
        Scan a ticket QR code at the entrance to verify and check in an attendee.
      </p>
        ...
        {preview && !previewLoading && (
          <div className="glass-card space-y-5 p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-xl font-bold">{preview.event.title}</h2>
            ...
            {checkIn.isSuccess ? (
              ...
              <button
                ...
                disabled={preview.status !== 'VALID' || checkIn.isPending}
